...
- **`-Hours N` is a request, not the window you get.** `--since` is unusable on the hub's Docker Desktop (GOTCHA #2), so the window is approximated by tail depth at ~8k lines/hour — the *hub's* peak rate. A quiet container produces far less, so the same tail reaches back much further: on the ai-01 sidecar, `-Hours 12` returned **186.6h** (8 days, 174 requests) under a header saying "last 12h". Over-pulling is safe for surveillance but the label is not, and a narrow `-Hours 1` re-check **cannot show a recent improvement** — it still returns the whole log. The script now prints the observed span and warns when it exceeds 1.5× the request; read that line, not the header.
