...
- **A restart is a stream killer, and the hub is almost never idle — so a restart should pick its moment rather than wait for silence.** `docker restart` cuts every in-flight SSE mid-body and each client reports `Connection lost mid-response` — that agent turn is lost. Measured over **53,959 `/health` samples spanning 33.8 continuous hours**: mean 3.6, p50 3, p90 6, max 15. The hub *does* fall idle — **4.6% of samples are at zero, in 772 episodes, one every ~2.6 min, median 2s and p90 11s** — so a 300s drain reaches a zero about **half** the time. `scripts/claudish-drain.ps1` observes for 60s to learn what quiet means right now, fires at the first sample at or below that minimum, and relaxes the target by one every 30s so the wait ends on a chosen moment rather than on the cap. Replaying all 53,959 samples, per restart: **blind 3.61 mean / 13 worst · wait-for-zero 2.36 / 15 · this 2.00 / 8**, with the share of restarts cutting nobody 4.8% → 49.1% → 23.9%. **Read that by the mean, not the last column**: wait-for-zero cuts nobody twice as often and still loses more turns, because its misses land anywhere. A hybrid holding out for zero until the last 60s scores 1.93 / 44.7% / worst 15 — inside the noise (300s windows 5s apart share nearly all their samples: ~405 independent windows, not 9,610), so it was not adopted. **Two drained restarts are now on record** (08-24 and 08-25, 04:00 local), measured from outside on a 2s `/health` probe: they cut **2 and 1** in-flight streams against 4.6 and 3.0 mean active over the preceding 60s, where the replay predicted 2.00. Across all **8 restarts in 36.6h the real cost is 3.00 turns each** — a fleet metric carried 6.4, which came from a stale second probe instance logging an NTP clock step as a restart. That does not isolate the wrapper, though: a graceful `docker stop` finishes in-flight streams by itself, so cutting below ambient is not by itself the drain's doing, and n=2 cannot separate the two. **Even at its best it cuts two agent turns, so the mitigation that actually matters is restarting less often, not draining better** — whether `ClaudishDailyRestart` (04:00, on the adaptive wrapper since 2026-08-23 — first drained restart 08-24 04:00, logged in `~/.claudish/drain.log`) needs to run at all is the open question, and moving the job to a quieter hour is **not** the answer: that idea rested on a single day, it was pre-registered, and the replication one day later refuted it — 05:00Z was 28.0% idle on 08-24 and **4.9%** on 08-25 (n=1,499 and 2,771; refuted at ≤5%), with no hour of the second day coming near the first day’s troughs. Which hours are quiet does not carry across days, which is exactly why the drain learns its target from the last 60s instead of assuming one. A `$null` count means *no signal* (proxy down, or image predates #37) and must degrade to an undrained restart, never to an unbounded wait.
...
| `compress-captures.ps1` · `claudish-watchdog.ps1` | nightly 7z compaction + GDrive backup (02:47 — moved off 04:17 on 2026-08-20: the 60-70 min run collided with the 04:00 container restart) · health check, proactive restart, hang recovery — **written for a 15-min schedule that has never been armed on any machine** (verified 2026-08-23: no `Get-ScheduledTask` entry, 4 log lines from 31 May). Since #37 it classifies probe failures (429/402/529 → DEGRADED, no restart) and only restarts proactively in the 03:00-06:00 quiet window |
| `claudish-drain.ps1` | **restart without dropping agent turns.** Run it standalone in place of `docker restart`, or dot-source it for `Invoke-ClaudishDrainedRestart` |
