...
- **A restart is a stream killer, and the hub is almost never idle — so a restart should pick its moment rather than wait for silence.** `docker restart` cuts every in-flight SSE mid-body and each client reports `Connection lost mid-response` — that agent turn is lost. Measured over **3080 `/health` samples spanning day and night**: p50 5, p90 8, max 14, mean 4.9. The hub *does* fall idle, but rarely and briefly — **five distinct silent episodes, one per ~88 min of observation, each 2 to 39 seconds** — so a 300s drain catches one roughly 1 time in 18. `scripts/claudish-drain.ps1` therefore observes for 60s to learn what quiet means right now, fires at the first sample at or below that minimum, and relaxes the target by one every 30s so the wait ends on a chosen moment rather than on the cap. Replaying all 3080 samples, per restart: **blind 4.86 mean / 11 worst · wait-for-zero 4.76 / 14 · this 3.03 / 7**, and the share of restarts cutting nobody goes 0.3% → 5.2% → 6.6%. Those defaults are the knee — a 600s cap scores the same as 300s, and relaxing once per poll instead of every 30s gives the gain straight back. **Even at its best it cuts three agent turns, so the mitigation that actually matters is restarting less often, not draining better** — whether `ClaudishDailyRestart` (04:00, repointed at the wrapper 2026-08-23) needs to run at all is the open question. A `$null` count means *no signal* (proxy down, or image predates #37) and must degrade to an undrained restart, never to an unbounded wait.
