...
- **A restart is a stream killer, and the hub is never idle — so a restart cannot wait for silence, only pick its moment.** `docker restart` cuts every in-flight SSE mid-body and each client reports `Connection lost mid-response` — that agent turn is lost. Measured on the hub 2026-08-23, **906 samples over 30 min: `activeStreams` never once reached 0** (min 1, seen twice; p50 5, p90 7, max 10, mean 5.1). A drain that waits for zero therefore only ever times out and then restarts at an arbitrary moment — which is what the first version shipped. `scripts/claudish-drain.ps1` instead observes for 60s to learn what quiet means right now, fires at the first sample at or below that minimum, and relaxes the target by one every 30s so the wait ends on a chosen moment rather than on the cap. Replaying the real samples: **mean 5.19 → 3.65, worst case 10 → 6**. Those defaults are the knee — a 600s cap scores the same as 300s, and relaxing once per poll instead of every 30s gives the gain straight back (4.67). **Because the floor is not zero, the mitigation that actually matters is restarting less often, not draining better** — `ClaudishDailyRestart` (04:00) has run the adaptive wrapper since 2026-08-23 (first drained restart: 08-24 04:00, logged in `~/.claudish/drain.log`), and whether it needs to run at all is the open question. A `$null` count means *no signal* (proxy down, or image predates #37) and must degrade to an undrained restart, never to an unbounded wait.
